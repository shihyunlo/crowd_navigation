...
\lhead{\normalsize \texttt{In} \textit{~Proceedings of \texttt{RSS'17} Workshop on Human Centered Robotics} \\
\texttt{Cambridge, MA, USA July 2017}}
\cfoot{}
...
%In particular, real-world deployment requires
...
%\commentp{Is the abstract still appropriate?  Do we still talk about blocking?}
\end{abstract}
...
%\commentp{what is "end-pose distance"?} 
to a large group 
of static people).
...
%\commentp{say whose crossing decisions} 
(considering 
pedestrian dynamics change at time $n$ in Eq.~\ref{eq:receding_control}
%\commentp{is that hte same n as in equation 1?  Do we really need a variable name here?}
), and the 
...
%\commentp{what is dynamical performance?} 
...
%\commentp{say what variables:  to identify appropriate values for all the variables that affect the pedestrian's motion (i.e.\ values that describe their observed motion so far)} 
...
%\commentp{how does it characterize the crossing scenario?  What does this mean?}  
$\xi^p_t$ is the past 
...
$Beta(p+1, q+1)$.
%\commentp{it's awkward to use Beta as the function and the variable...} 
...
%\commentp{what does it mean to learn the model?  What parameters nened to be set, and how do you characterize success?} 
...
%\commentp{specify what is who is predicting what} 
...
%\commentp{the robot to predict the pedestrian's motion, taking into account} 
based on the 
robot's crossing action, and facilitates realistic look-ahead planning by the robot.
%\commentp{by the robot}.
...
%\commentp{It's strange to say that without having first described the experiments.}
...
around.
%\commentp{I don't understand this last sentence}
...
%\commentp{I still have no idea what the point of this paragraph is.}
%\commentl{I was to explain that people have different reactions to different 
%opponent, which is modeled in the process}
%\commentp{Don't you say that in the next paragraph?}
%Our subject, human crossing behavior in crowds, is considered as the reaction 
%to potential threat, which has longer response time compared to other 
%stimulus, and is influenced by the opponent's behavior as well as intention.
%%(which we modeled in Eq.~\ref{eq:receding_control} as variable $n$)
%This human crossing path decision is modeled to rely on assumptions about 
%opponent, such that the person can maximize his/her own utility. 

%\commentp{I don't undrestand this paragraph}
...
%With the above models and motivation in mind, we gathered data to study the types of human n%avigation behaviors that tend to emerge when interacting with real robots.
...
%  response?
%\end{enumerate}
%\commentp{I don't understand either of these sentences.  Can you frame them exactly in the terms of the rest of the paper?  What variables/categories are we trying to find empirically?}
...
%\commentp{How is the frequency of intention communication measured?}
...
%\commentp{You don't say how you categorize people in this way.  Based on what features?}
%Due to space constraint, graphical illustration of preliminary results are 
%shown in the workshop session for discussion. 
%\commentp{I don't think we need that last sentence.  Replace with a sentence about ongoing work.}
%We observed only people who assume the 
%robot to be safe (such as comfortable people) appear interactive with the 
%robot during crossing. Their paths exhibit larger variance. Other participants 
%(such as cautious or aggressive people) navigate straight to their goals 
%with non-interactive routes, such as routes with large safety margins. Those 
%people also tend to react to their crossing decision 
